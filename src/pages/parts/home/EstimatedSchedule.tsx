...
export function EstimatedSchedule() {
  ...
  const groupedByDate = scheduleItems.reduce<Record<string, ScheduleItem[]>>(
    (acc, item) => {
        ...
        <div className="grid gap-6">
          {Object.entries(groupedByDate).map(([date, items]) => (
            <div key={date} className="space-y-3">
              <h3 className="text-lg font-semibold text-type-emphasis">
                {date}
              </h3>
              ...
              <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                {items.map((item) => (
                  <div
                    key={item.id}
                    className="bg-largeCard-background rounded-lg p-4 border border-type-divider transition-colors duration-200 hover:border-type-emphasis hover:scale-105"
                  >
                    ...
                    <div className="mt-3 border-t border-type-divider pt-3">
